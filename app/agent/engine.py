from datetime import datetime, timezone

from app.agent.reasoner import RuleBasedReasoner
from app.config import settings
from app.models import AgentRunResult
from app.services.dedup import is_duplicate
from app.services.discovery import get_discovery_provider
from app.services.parser import extract_signals
from app.services.reporting import create_daily_report
from app.services.storage import create_lead, create_run_log


class LeadAgentEngine:
    def __init__(self) -> None:
        self.reasoner = RuleBasedReasoner()

    @staticmethod
    def _first_or_none(items):
        if isinstance(items, list) and items:
            return items[0]
        return None

    def run_cycle(self) -> AgentRunResult:
        started_at = datetime.now(timezone.utc)
        leads_saved = 0
        high_conf = 0
        errors: list[str] = []

        try:
            provider = get_discovery_provider(settings.discovery_mode)
            candidates = provider.discover() or []
        except Exception as exc:
            candidates = []
            errors.append(f"discovery_error: {exc}")

        for idx, candidate in enumerate(candidates):
            try:
                if not candidate:
                    errors.append(f"candidate_{idx}: empty candidate")
                    continue

                evidence = extract_signals(
                    getattr(candidate, "html", ""),
                    getattr(candidate, "website", ""),
                    getattr(candidate, "page_text", ""),
                )
                reasoning = self.reasoner.evaluate(evidence)

                if reasoning.confidence_score >= 0.8:
                    high_conf += 1

                if not reasoning.save_decision:
                    continue

                website = getattr(candidate, "website", None)
                company_name = getattr(candidate, "company_name", None)
                if is_duplicate(website, company_name):
                    continue

                lead_payload = {
                    "company_name": company_name,
                    "website": website,
                    "country": "UAE" if evidence.get("uae_cities") else None,
                    "city": self._first_or_none(evidence.get("uae_cities")),
                    "phone": self._first_or_none(evidence.get("phones")),
                    "public_email": self._first_or_none(evidence.get("emails")),
                    "contact_page": evidence.get("contact_page"),
                    "careers_page": evidence.get("careers_page"),
                    "lead_type": reasoning.classification,
                    "confidence_score": reasoning.confidence_score,
                    "source_url": getattr(candidate, "source_url", None),
                    "reasoning": reasoning.explanation,
                    "detected_signals": reasoning.detected_signals,
                    "notes": "Autonomously discovered from public web content.",
                }
                create_lead(lead_payload)
                leads_saved += 1
            except Exception as exc:  # never fail entire run due to one candidate
                errors.append(f"candidate_{idx}_error: {exc}")
                continue

        finished_at = datetime.now(timezone.utc)
        status = "partial" if errors else "success"
        error_summary = f" Errors: {' | '.join(errors[:5])}" if errors else ""
        summary = f"Processed {len(candidates)} candidates and saved {leads_saved} leads.{error_summary}"

        create_run_log(
            {
                "started_at": started_at.isoformat(),
                "finished_at": finished_at.isoformat(),
                "status": status,
                "candidates_seen": len(candidates),
                "leads_saved": leads_saved,
                "summary": summary,
            }
        )

        create_daily_report(
            {
                "report_date": started_at.date().isoformat(),
                "total_candidates": len(candidates),
                "total_saved": leads_saved,
                "high_confidence_count": high_conf,
                "summary_text": summary,
            }
        )

        return AgentRunResult(
            started_at=started_at,
            finished_at=finished_at,
            candidates_seen=len(candidates),
            leads_saved=leads_saved,
            high_confidence_count=high_conf,
            status=status,
            summary=summary,
        )
